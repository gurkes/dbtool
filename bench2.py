	...
	try:
		...
		if prepare:
			...
			cmdprepare = "/usr/bin/sysbench %s --mysql-host=%s --mysql-port=%s --mysql-user=%s --mysql-password='%s' --mysql-db=%s --db-driver=mysql --tables=%s --table-size=%s --time=%s --max-requests=%s prepare" % (
				info['sysbench']['lua_script'], host, port, user, password, dbname, info['sysbench']['table_amount'],
				info['sysbench']['rows'],
			...
			while count <= runtimes:
				for thread in threadnumber:
					...
					lg(msg)
					cmdsysbench = "sysbench %s --mysql-host=%s --mysql-port=%s --mysql-user=%s --mysql-password=%s --mysql-db=%s --db-driver=mysql --tables=%s --table-size=%s --report-interval=%s --threads=%s --rand-type=uniform --time=%s --max-requests=%s --mysql-ignore-errors=1062,1213 run >> %s" % (
					info['sysbench']['lua_script'], host, port, user, password, dbname,
					info['sysbench']['table_amount'], info['sysbench']['rows'], info['sysbench']['interval'], thread,
					info['sysbench']['exectime'], info['sysbench']['max_request'], logfile)
					...
					if state == None:
						...
						lg(msg)
						if outformat:
							#cmdout='cat %s |grep -E "transactions:|queries:|ignored errors:|reconnects:|min:|avg:|max:|95th percentile:"|awk \'{if($1~/transactions|queries|reconnects|95th/){print $3}else if($1~/max|avg|min/){print $2}else if($1~/ignored/){print $4}}\'|xargs' % (logfile)
							cmdout='cat %s |grep -E "transactions:|queries:|ignored errors:|reconnects:|min:|avg:|max:|95th percentile:"|awk \'{if($1~/transactions|queries|reconnects|95th/){print $3}else if($1~/max|avg|min/){print $2}else if($1~/ignored/){print $4}}\'|sed s/\\(//g|xargs|sed "s/ /\\t/g"' % (logfile)
							#print cmdout
							pipe=os.popen(cmdout)
							print pipe.readlines()[0].rstrip("\n")
							#pipe.close()
					else:
...
if __name__ == "__main__":
	...
	parser.add_argument('-f', '--outformat', action='store_true', default=False, help='format output')
